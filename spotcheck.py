...
MAIN_MENU_BUTTON_FONT = ('Arial', 11, 'bold')
LABELFRAME_TXT_FONT = ('Arial', 13)
MAIN_FUCNTION_BUTTON_FONT = ('Arial', 11)
TITLE_TXT_FONT = ('Arial', 11, 'bold')
SWITCH_PAGE_BUTTON_FONT = ('Arial', 11)
LABEL_FRAME_TXT_FONT = ('Arial', 11)
LABEL_TXT_FONT = ('Arial', 11)
ENTRY_TXT_FONT = ('Arial', 12)
CONFIRM_BUTTON_TXT_FONT = ('Arial', 11)
SAMPLE_BUTTON_TXT_FONT = ('Arial', 11)
SAMPLE_LABEL_TXT_FONT = ('Arial', 14)
RESULT_LABEL_TXT_FONT = ('Arial', 11)
RESULT_LABEL_TXT_FONT_1 = ('Arial', 11, 'bold')
PROGRAM_BUTTON_TXT_FONT = ('Arial', 11)
LOGIN_LABEL_TXT_FONT = ('Arial', 16)
LOGIN_BUTTON_TXT_FONT = ('Arial', 11)
...
Screening3_Language = {
	...
	"Environment RadioButton": ["Environment", "Môi trường"],
...
Quantitative3_Language = {
	...
	"Note CT Label": ["CT", "CT"],
	"Note Negative Label": ["NEGATIVE (N)", "ÂM TÍNH"],
	"Note LowCopy Label": ["LOW COPY (P_L)", "BẢN SAO THẤP (P_L)"],
	...
	"Result SampleName Text": ["Sample name", "Tên mẫu"],
	"Result SamplePosition Text": ["Position", "Vị trí"],
	"Result SCResult Text": ["Spotcheck Result (Copy)", "Kết quả Spotcheck (Copy)"],
	"Result CTResult Text": ["Equivalent CT", "CT tương đương"],
	"Result GelResult Text": ["Gel Result", "Kết quả gel"],
	"Result FinalResult Text": ["Final Result", "Kết luận"],
	"FinalTitle Label": ["ANALYSIS RESULTS", "KẾT QUẢ PHÂN TÍCH"],
...
SystemCheck_Language = {
	...
	"ZeroLight Error": ["Unable to check the system (If there is a protective plate on the sample tray, please remove it and restart the application)",
...
class ScrollableFrame1(Frame):
	def __init__(self, container, *args, **kwargs):
		super().__init__(container, *args, **kwargs)
		canvas = Canvas(self, bg = MAIN_FUNCTION_FRAME_BGD_COLOR, height=480, width=650)
		scrollbar = ttk.Scrollbar(self, orient="vertical", command=canvas.yview)
		self.scrollable_frame = Frame(canvas, bg = MAIN_FUNCTION_FRAME_BGD_COLOR)
...
class ScrollableFrame2(Frame):
	def __init__(self, container, *args, **kwargs):
		super().__init__(container, *args, **kwargs)
		canvas = Canvas(self, bg = MAIN_FUNCTION_FRAME_BGD_COLOR, height=480, width=800)
		scrollbar = ttk.Scrollbar(self, orient="vertical", command=canvas.yview)
		self.scrollable_frame = Frame(canvas, bg = MAIN_FUNCTION_FRAME_BGD_COLOR)
...
class ScrollableFrame3(Frame):
	def __init__(self, container, *args, **kwargs):
		super().__init__(container, *args, **kwargs)
		canvas = Canvas(self, bg = MAIN_FUNCTION_FRAME_BGD_COLOR, height=410, width=370)
		scrollbar = ttk.Scrollbar(self, orient="vertical", command=canvas.yview)
		self.scrollable_frame = Frame(canvas, bg = MAIN_FUNCTION_FRAME_BGD_COLOR)
	...
	def serial_handle(self):
		...
		if(bled_ready):
			...
			try:
				...
				fw_instance.writelines('0\n')
				fw_instance.close()
				msg = messagebox.showerror("ERR "+ str(ERROR_LIST['SYSTEM_ERROR_3'].value),
										ERROR_LIST(ERROR_LIST['SYSTEM_ERROR_3'].value).name + '\n' + SystemCheck_Language["ZeroLight Error"][language],
										icon = "error")
				os._exit(0)
				self.base_window.destroy()
...
class QualitativeAnalysisFrame3(Frame):
	...
	def serial_handle(self):
		...
		if(bled_ready):
			...
			sheet["B5"] = Screening3_Language["Result Title Text"][language]
			sheet["B5"].font = font1
			sheet.cell(row=5,column=2).alignment = Alignment(horizontal='center',vertical='center',wrapText=True)
			sheet["B7"] = Screening3_Language["Result ExperimentName Text"][language] + self.base_window.qualitative_analysis_0.experiment_name
			sheet["B7"].font = font2
			sheet['B8'] = Screening3_Language["Result TechnicianName Text"][language] + self.base_window.qualitative_analysis_0.user_name
			sheet["B8"].font = font2
			sheet['B9'] = Screening3_Language["Result Date Text"][language] + self.base_window.qualitative_analysis_0.create_time
			sheet["B9"].font = font2
			...
			sheet['B64'] = Screening3_Language["Result TechnicianNameSign Text"][language]
			sheet['B65'] = ''
			sheet['D64'] = Screening3_Language["Result HeadOfDivisionSign Text"][language]
			sheet['D65'] = ''
	...
	def update_result(self):
		...
		if(self.quantitative_view == 0):
			...
			self.check_result_frame = Frame(self.result_tab, bg=RESULT_TABLE_FRAME_BGD_COLOR)
			self.check_result_frame.grid(row=0, column=0, padx=10)
...
class QuantitativeAnalysisFrame3(QualitativeAnalysisFrame3):
	...
	def serial_handle(self):
		...
		if(bled_ready):
			...
			value_text_label.grid(row=0, column=0, padx=5, pady=8)
			...
			############## Cat bo vi tri N/A ###############
			first_result_row = RESULT_CELL_START
			last_result_row = first_result_row + SC_VERSION*2
			row = first_result_row
			tmp_row_value = first_result_row + SC_VERSION - 1
...
class QualitativeAnalysisFrame2(Frame):
	def __init__(self, container):
		...
		self.id_load_frame.grid(row=0, column=0, pady=192, padx=50)
			...
			if(self.base_window.email_setting.account_active == 1):
				...
				if(msg=='yes'):
					self.email_label_frame = LabelFrame(self.work_frame,
														...
														bg = 'grey75')
					self.email_label_frame.place(x=235, y=177)
...
class QuantitativeAnalysisFrame1(Frame):
	def __init__(self, container):
		...
		# In work frame
		self.program_frame = ScrollableFrame(self.work_frame)
		self.program_frame.grid(row=0, column=0, pady=124)
		...
		if(msg=="yes"):
			...
			if(file_create_done):
				if(self.direct_create == 0): #create file with create module from main menu
					...
					sheet = wb.active			
					if(SC_VERSION == 48):		
						self.base_window.qualitative_analysis_2.id_list[0] = sheet["B" + str(RESULT_CELL_START)].value
						self.base_window.qualitative_analysis_2.id_list[1] = sheet["B" + str(RESULT_CELL_START+8)].value
						self.base_window.qualitative_analysis_2.id_list[2] = sheet["B" + str(RESULT_CELL_START+16)].value
						self.base_window.qualitative_analysis_2.id_list[3] = sheet["B" + str(RESULT_CELL_START+24)].value
						self.base_window.qualitative_analysis_2.id_list[4] = sheet["B" + str(RESULT_CELL_START+32)].value
						self.base_window.qualitative_analysis_2.id_list[5] = sheet["B" + str(RESULT_CELL_START+40)].value
						
						self.base_window.qualitative_analysis_2.id_list[6] = sheet["B" + str(RESULT_CELL_START+1)].value
						self.base_window.qualitative_analysis_2.id_list[7] = sheet["B" + str(RESULT_CELL_START+9)].value
						self.base_window.qualitative_analysis_2.id_list[8] = sheet["B" + str(RESULT_CELL_START+17)].value
						self.base_window.qualitative_analysis_2.id_list[9] = sheet["B" + str(RESULT_CELL_START+25)].value
						self.base_window.qualitative_analysis_2.id_list[10] = sheet["B" + str(RESULT_CELL_START+33)].value
						self.base_window.qualitative_analysis_2.id_list[11] = sheet["B" + str(RESULT_CELL_START+41)].value
						
						self.base_window.qualitative_analysis_2.id_list[12] = sheet["B" + str(RESULT_CELL_START+2)].value
						self.base_window.qualitative_analysis_2.id_list[13] = sheet["B" + str(RESULT_CELL_START+10)].value
						self.base_window.qualitative_analysis_2.id_list[14] = sheet["B" + str(RESULT_CELL_START+18)].value
						self.base_window.qualitative_analysis_2.id_list[15] = sheet["B" + str(RESULT_CELL_START+26)].value
						self.base_window.qualitative_analysis_2.id_list[16] = sheet["B" + str(RESULT_CELL_START+34)].value
						self.base_window.qualitative_analysis_2.id_list[17] = sheet["B" + str(RESULT_CELL_START+42)].value
						
						self.base_window.qualitative_analysis_2.id_list[18] = sheet["B" + str(RESULT_CELL_START+3)].value
						self.base_window.qualitative_analysis_2.id_list[19] = sheet["B" + str(RESULT_CELL_START+11)].value
						self.base_window.qualitative_analysis_2.id_list[20] = sheet["B" + str(RESULT_CELL_START+19)].value
						self.base_window.qualitative_analysis_2.id_list[21] = sheet["B" + str(RESULT_CELL_START+27)].value
						self.base_window.qualitative_analysis_2.id_list[22] = sheet["B" + str(RESULT_CELL_START+35)].value
						self.base_window.qualitative_analysis_2.id_list[23] = sheet["B" + str(RESULT_CELL_START+43)].value
						
						self.base_window.qualitative_analysis_2.id_list[24] = sheet["B" + str(RESULT_CELL_START+4)].value
						self.base_window.qualitative_analysis_2.id_list[25] = sheet["B" + str(RESULT_CELL_START+12)].value
						self.base_window.qualitative_analysis_2.id_list[26] = sheet["B" + str(RESULT_CELL_START+20)].value
						self.base_window.qualitative_analysis_2.id_list[27] = sheet["B" + str(RESULT_CELL_START+28)].value
						self.base_window.qualitative_analysis_2.id_list[28] = sheet["B" + str(RESULT_CELL_START+36)].value
						self.base_window.qualitative_analysis_2.id_list[29] = sheet["B" + str(RESULT_CELL_START+44)].value
						
						self.base_window.qualitative_analysis_2.id_list[30] = sheet["B" + str(RESULT_CELL_START+5)].value
						self.base_window.qualitative_analysis_2.id_list[31] = sheet["B" + str(RESULT_CELL_START+13)].value
						self.base_window.qualitative_analysis_2.id_list[32] = sheet["B" + str(RESULT_CELL_START+21)].value
						self.base_window.qualitative_analysis_2.id_list[33] = sheet["B" + str(RESULT_CELL_START+29)].value
						self.base_window.qualitative_analysis_2.id_list[34] = sheet["B" + str(RESULT_CELL_START+37)].value
						self.base_window.qualitative_analysis_2.id_list[35] = sheet["B" + str(RESULT_CELL_START+45)].value
						
						self.base_window.qualitative_analysis_2.id_list[36] = sheet["B" + str(RESULT_CELL_START+6)].value
						self.base_window.qualitative_analysis_2.id_list[37] = sheet["B" + str(RESULT_CELL_START+14)].value
						self.base_window.qualitative_analysis_2.id_list[38] = sheet["B" + str(RESULT_CELL_START+22)].value
						self.base_window.qualitative_analysis_2.id_list[39] = sheet["B" + str(RESULT_CELL_START+30)].value
						self.base_window.qualitative_analysis_2.id_list[40] = sheet["B" + str(RESULT_CELL_START+38)].value
						self.base_window.qualitative_analysis_2.id_list[41] = sheet["B" + str(RESULT_CELL_START+46)].value
						
						self.base_window.qualitative_analysis_2.id_list[42] = sheet["B" + str(RESULT_CELL_START+7)].value
						self.base_window.qualitative_analysis_2.id_list[43] = sheet["B" + str(RESULT_CELL_START+15)].value
						self.base_window.qualitative_analysis_2.id_list[44] = sheet["B" + str(RESULT_CELL_START+23)].value
						self.base_window.qualitative_analysis_2.id_list[45] = sheet["B" + str(RESULT_CELL_START+31)].value
						self.base_window.qualitative_analysis_2.id_list[46] = sheet["B" + str(RESULT_CELL_START+39)].value
						self.base_window.qualitative_analysis_2.id_list[47] = sheet["B" + str(RESULT_CELL_START+47)].value
					else:
						self.base_window.qualitative_analysis_2.id_list[0] = sheet["B" + str(RESULT_CELL_START)].value
						self.base_window.qualitative_analysis_2.id_list[1] = sheet["B" + str(RESULT_CELL_START+4)].value
						self.base_window.qualitative_analysis_2.id_list[2] = sheet["B" + str(RESULT_CELL_START+8)].value
						self.base_window.qualitative_analysis_2.id_list[3] = sheet["B" + str(RESULT_CELL_START+12)].value
						
						self.base_window.qualitative_analysis_2.id_list[4] = sheet["B" + str(RESULT_CELL_START+1)].value
						self.base_window.qualitative_analysis_2.id_list[5] = sheet["B" + str(RESULT_CELL_START+5)].value
						self.base_window.qualitative_analysis_2.id_list[6] = sheet["B" + str(RESULT_CELL_START+9)].value
						self.base_window.qualitative_analysis_2.id_list[7] = sheet["B" + str(RESULT_CELL_START+13)].value

						self.base_window.qualitative_analysis_2.id_list[8] = sheet["B" + str(RESULT_CELL_START+2)].value
						self.base_window.qualitative_analysis_2.id_list[9] = sheet["B" + str(RESULT_CELL_START+6)].value
						self.base_window.qualitative_analysis_2.id_list[10] = sheet["B" + str(RESULT_CELL_START+10)].value
						self.base_window.qualitative_analysis_2.id_list[11] = sheet["B" + str(RESULT_CELL_START+11)].value

						self.base_window.qualitative_analysis_2.id_list[12] = sheet["B" + str(RESULT_CELL_START+3)].value
						self.base_window.qualitative_analysis_2.id_list[13] = sheet["B" + str(RESULT_CELL_START+7)].value
						self.base_window.qualitative_analysis_2.id_list[14] = sheet["B" + str(RESULT_CELL_START+11)].value
						self.base_window.qualitative_analysis_2.id_list[15] = sheet["B" + str(RESULT_CELL_START+12)].value
					...
					sheet = wb.active					
					if(SC_VERSION == 48):		
						self.base_window.quantitative_analysis_2.id_list[0] = sheet["B" + str(RESULT_CELL_START)].value
						self.base_window.quantitative_analysis_2.id_list[1] = sheet["B" + str(RESULT_CELL_START+8)].value
						self.base_window.quantitative_analysis_2.id_list[2] = sheet["B" + str(RESULT_CELL_START+16)].value
						self.base_window.quantitative_analysis_2.id_list[3] = sheet["B" + str(RESULT_CELL_START+24)].value
						self.base_window.quantitative_analysis_2.id_list[4] = sheet["B" + str(RESULT_CELL_START+32)].value
						self.base_window.quantitative_analysis_2.id_list[5] = sheet["B" + str(RESULT_CELL_START+40)].value
						
						self.base_window.quantitative_analysis_2.id_list[6] = sheet["B" + str(RESULT_CELL_START+1)].value
						self.base_window.quantitative_analysis_2.id_list[7] = sheet["B" + str(RESULT_CELL_START+9)].value
						self.base_window.quantitative_analysis_2.id_list[8] = sheet["B" + str(RESULT_CELL_START+17)].value
						self.base_window.quantitative_analysis_2.id_list[9] = sheet["B" + str(RESULT_CELL_START+25)].value
						self.base_window.quantitative_analysis_2.id_list[10] = sheet["B" + str(RESULT_CELL_START+33)].value
						self.base_window.quantitative_analysis_2.id_list[11] = sheet["B" + str(RESULT_CELL_START+41)].value
						
						self.base_window.quantitative_analysis_2.id_list[12] = sheet["B" + str(RESULT_CELL_START+2)].value
						self.base_window.quantitative_analysis_2.id_list[13] = sheet["B" + str(RESULT_CELL_START+10)].value
						self.base_window.quantitative_analysis_2.id_list[14] = sheet["B" + str(RESULT_CELL_START+18)].value
						self.base_window.quantitative_analysis_2.id_list[15] = sheet["B" + str(RESULT_CELL_START+26)].value
						self.base_window.quantitative_analysis_2.id_list[16] = sheet["B" + str(RESULT_CELL_START+34)].value
						self.base_window.quantitative_analysis_2.id_list[17] = sheet["B" + str(RESULT_CELL_START+42)].value
						
						self.base_window.quantitative_analysis_2.id_list[18] = sheet["B" + str(RESULT_CELL_START+3)].value
						self.base_window.quantitative_analysis_2.id_list[19] = sheet["B" + str(RESULT_CELL_START+11)].value
						self.base_window.quantitative_analysis_2.id_list[20] = sheet["B" + str(RESULT_CELL_START+19)].value
						self.base_window.quantitative_analysis_2.id_list[21] = sheet["B" + str(RESULT_CELL_START+27)].value
						self.base_window.quantitative_analysis_2.id_list[22] = sheet["B" + str(RESULT_CELL_START+35)].value
						self.base_window.quantitative_analysis_2.id_list[23] = sheet["B" + str(RESULT_CELL_START+43)].value
						
						self.base_window.quantitative_analysis_2.id_list[24] = sheet["B" + str(RESULT_CELL_START+4)].value
						self.base_window.quantitative_analysis_2.id_list[25] = sheet["B" + str(RESULT_CELL_START+12)].value
						self.base_window.quantitative_analysis_2.id_list[26] = sheet["B" + str(RESULT_CELL_START+20)].value
						self.base_window.quantitative_analysis_2.id_list[27] = sheet["B" + str(RESULT_CELL_START+28)].value
						self.base_window.quantitative_analysis_2.id_list[28] = sheet["B" + str(RESULT_CELL_START+36)].value
						self.base_window.quantitative_analysis_2.id_list[29] = sheet["B" + str(RESULT_CELL_START+44)].value
						
						self.base_window.quantitative_analysis_2.id_list[30] = sheet["B" + str(RESULT_CELL_START+5)].value
						self.base_window.quantitative_analysis_2.id_list[31] = sheet["B" + str(RESULT_CELL_START+13)].value
						self.base_window.quantitative_analysis_2.id_list[32] = sheet["B" + str(RESULT_CELL_START+21)].value
						self.base_window.quantitative_analysis_2.id_list[33] = sheet["B" + str(RESULT_CELL_START+29)].value
						self.base_window.quantitative_analysis_2.id_list[34] = sheet["B" + str(RESULT_CELL_START+37)].value
						self.base_window.quantitative_analysis_2.id_list[35] = sheet["B" + str(RESULT_CELL_START+45)].value
						
						self.base_window.quantitative_analysis_2.id_list[36] = sheet["B" + str(RESULT_CELL_START+6)].value
						self.base_window.quantitative_analysis_2.id_list[37] = sheet["B" + str(RESULT_CELL_START+14)].value
						self.base_window.quantitative_analysis_2.id_list[38] = sheet["B" + str(RESULT_CELL_START+22)].value
						self.base_window.quantitative_analysis_2.id_list[39] = sheet["B" + str(RESULT_CELL_START+30)].value
						self.base_window.quantitative_analysis_2.id_list[40] = sheet["B" + str(RESULT_CELL_START+38)].value
						self.base_window.quantitative_analysis_2.id_list[41] = sheet["B" + str(RESULT_CELL_START+46)].value
						
						self.base_window.quantitative_analysis_2.id_list[42] = sheet["B" + str(RESULT_CELL_START+7)].value
						self.base_window.quantitative_analysis_2.id_list[43] = sheet["B" + str(RESULT_CELL_START+15)].value
						self.base_window.quantitative_analysis_2.id_list[44] = sheet["B" + str(RESULT_CELL_START+23)].value
						self.base_window.quantitative_analysis_2.id_list[45] = sheet["B" + str(RESULT_CELL_START+31)].value
						self.base_window.quantitative_analysis_2.id_list[46] = sheet["B" + str(RESULT_CELL_START+39)].value
						self.base_window.quantitative_analysis_2.id_list[47] = sheet["B" + str(RESULT_CELL_START+47)].value
					else:
						self.base_window.quantitative_analysis_2.id_list[0] = sheet["B" + str(RESULT_CELL_START)].value
						self.base_window.quantitative_analysis_2.id_list[1] = sheet["B" + str(RESULT_CELL_START+4)].value
						self.base_window.quantitative_analysis_2.id_list[2] = sheet["B" + str(RESULT_CELL_START+8)].value
						self.base_window.quantitative_analysis_2.id_list[3] = sheet["B" + str(RESULT_CELL_START+12)].value
						
						self.base_window.quantitative_analysis_2.id_list[4] = sheet["B" + str(RESULT_CELL_START+1)].value
						self.base_window.quantitative_analysis_2.id_list[5] = sheet["B" + str(RESULT_CELL_START+5)].value
						self.base_window.quantitative_analysis_2.id_list[6] = sheet["B" + str(RESULT_CELL_START+9)].value
						self.base_window.quantitative_analysis_2.id_list[7] = sheet["B" + str(RESULT_CELL_START+13)].value

						self.base_window.quantitative_analysis_2.id_list[8] = sheet["B" + str(RESULT_CELL_START+2)].value
						self.base_window.quantitative_analysis_2.id_list[9] = sheet["B" + str(RESULT_CELL_START+6)].value
						self.base_window.quantitative_analysis_2.id_list[10] = sheet["B" + str(RESULT_CELL_START+10)].value
						self.base_window.quantitative_analysis_2.id_list[11] = sheet["B" + str(RESULT_CELL_START+14)].value

						self.base_window.quantitative_analysis_2.id_list[12] = sheet["B" + str(RESULT_CELL_START+3)].value
						self.base_window.quantitative_analysis_2.id_list[13] = sheet["B" + str(RESULT_CELL_START+7)].value
						self.base_window.quantitative_analysis_2.id_list[14] = sheet["B" + str(RESULT_CELL_START+11)].value
						self.base_window.quantitative_analysis_2.id_list[15] = sheet["B" + str(RESULT_CELL_START+15)].value
...
class EmailSettingFrame(Frame):
	def __init__(self, container):
		...
		self.user_entry = Entry(self.work_frame, width=30, justify='right', font=('Arial',14))
		self.user_entry.grid(row=1, column=1, sticky=W)
		self.pass_entry = Entry(self.work_frame, width=30, show='◼', justify='right', font=('Arial',14))
...
class ViewResultFrame(Frame):
	def __init__(self, container):
		...
		self.infor_frame.pack(side=LEFT, anchor=W, pady=5, padx=20)

		self.report_frame = ScrollableFrame1(self.work_frame)
		...
		self.open_button.pack(ipadx=30, ipady=10, side=LEFT, padx=330)
		...
		if self.path is not None:
			try:
				...
				self.infor_frame = LabelFrame(self.work_frame,
								...
								width=200,
								text = ViewResult_Language['Information LabelFrame'][language])
				self.infor_frame.pack(side=LEFT, anchor=N, pady=5, padx=20)
			except:
				pass
			...
			info1_button_list = Button(self.infor_frame,
					fg = LABEL_TXT_COLOR,
					font = LABEL_TXT_FONT,
					text= sheet["B" + str(RESULT_CELL_START-5)].value,
					width=30,
					bg = 'lavender',
					borderwidth = 0)
			...
			info2_button_list = Button(self.infor_frame,
					fg = LABEL_TXT_COLOR,
					font = LABEL_TXT_FONT,
					text= sheet["B" + str(RESULT_CELL_START-4)].value,
					width=30,
					bg = 'lavender',
					borderwidth = 0)
			...
			info3_button_list = Button(self.infor_frame,
					fg = LABEL_TXT_COLOR,
					font = LABEL_TXT_FONT,
					text= sheet["B" + str(RESULT_CELL_START-3)].value,
					width=30,
					bg = 'lavender',
					borderwidth = 0)
			...
			for i in range(0,SC_VERSION + 1):
				sample_pos = 'B' + str(RESULT_CELL_START-1 +i)
				if(sheet[sample_pos].value == None):
					break
				sample_button_list[i] = Button(self.report_frame.scrollable_frame,
						fg = LABEL_TXT_COLOR,
						font = LABEL_TXT_FONT,
				...
				position_pos = 'C' + str(RESULT_CELL_START-1 +i)
				position_button_list[i] = Button(self.report_frame.scrollable_frame,
						fg = LABEL_TXT_COLOR,
						font = LABEL_TXT_FONT,
						text= sheet[position_pos].value,
				...
				result_pos = 'D' + str(RESULT_CELL_START-1 +i)
				result_button_list[i] = Button(self.report_frame.scrollable_frame,
						fg = LABEL_TXT_COLOR,
						font = LABEL_TXT_FONT,
						text= sheet[result_pos].value,
						...
						bg = 'lavender',
						borderwidth = 0)
				result_button_list[i].grid(row=i, column=2, sticky=EW, padx=1, pady=1)
			wb.close()
...
class QualitativeOptionFrame(Frame):
	def __init__(self, container):
		...
		self.calibration_button.grid(row=0, column=0, ipadx=40, ipady=20, padx=80, pady=224)
		...
		self.analysis_button.grid(row=0, column=1, ipadx=40, ipady=20, padx=80, pady=224)
...
class MainMenu(Frame):
	def __init__(self, master):
		...
		self.language_combobox['values'] = ('English',
											'Tiếng Việt')
		self.language_combobox.current(language)
		self.language_combobox.place(x=1024-101, y=1)
